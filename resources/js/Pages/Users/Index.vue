...
watch(form, throttle(function() {
        Inertia.get(route('users.index'), pickBy(form), { preserveState: true })
      }, 900));
                ...
                <div>
                    <div class="flex mb-4 w-full justify-between">
                        ...
                        <button
                            ...
                            @click="Inertia.visit(route('users.create'))">Add New</button>
                            ...
                            <tr v-for="user in users.data" :key="user.id" class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span class="flex flex-col">
                                        ...
                                        <span class=" font-bold text-xs">{{user.phone_number }}</span>
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    <p>
                                        <span class=" p-1 text-xs bg-orange-100 text-orange-500 rounded" v-if="user?.created_at">{{ dayjs(user?.created_at).fromNow() }} </span>
                                    </p>
                                    <span class=" p-1 text-xs bg-orange-100 text-orange-500 rounded" v-if="user?.email_verified_at">{{ dayjs(user?.email_verified_at).fromNow() }} </span>
                                    <!--span class=" p-1 text-xs bg-blue-100 text-blue-500 rounded" v-if="user.status==1">Paid</span>
              <span class=" p-1 text-xs bg-yellow-100 text-yellow-500 rounded" v-if="user.status==2">Planed</span>
              <span class=" p-1 text-xs bg-green-100 text-green-500 rounded" v-if="user.status==3">Delivered</span>
              <span class=" p-1 text-xs bg-gray-100 text-gray-600 rounded" v-if="user.status==5">Rembourser</span--->
                                </td>
                                <td class="px-4 py-3 text-sm">

                                    <!--Link :href="route('users.show',user.id)" active
